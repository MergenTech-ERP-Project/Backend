...
public interface UserMRepository extends JpaRepository<UserM,Long> {
    ...
    @Query(value = "SELECT * from public.user_m u Where u.username = ?1 and u.password = ?2", nativeQuery = true)
    ...
    @Query(value = "SELECT * from public.user_m u Where u.email = ?1 and u.username = ?2", nativeQuery = true)
